...
def rcspp(G, s, t, l):
    ...
    model.addCons( ( quicksum(x[a] for a in out_arcs[s]) - quicksum(x[a] for a in in_arcs[s]) ) == 1)
    model.addCons( ( quicksum(x[a] for a in out_arcs[t]) - quicksum(x[a] for a in in_arcs[t]) ) == -1)
    for u in vertices:
        if u != s and u != t:
            model.addCons( ( quicksum(x[a] for a in out_arcs[u]) - quicksum(x[a] for a in in_arcs[u]) ) == 0)
    # resource constraint:
...
def create_path(x, start):
    """
    unction that converts the model values to a path array
    """
...
if __name__ == "__main__":
    ...
    instances = ["data/graph1_2.txt", "data/graph4_2.txt", "data/grid1.txt", "data/grid2.txt"]
    ...
    for inst in instances:
        ...
        print("\nA shortest "+str(s)+"-"+str(t)+"-path with resource consumption ≤", l, "is\n", path)
        print("with length", opt_val, "and resource consumption", cons, ".")
        print("The computation took ", "%.1f" % runtime, "seconds of which", "%.1f" % solvetime, "seconds were used to solve the model.")